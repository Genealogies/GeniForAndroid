...
class TypeView(context: Context, combo: Combo?) : AppCompatAutoCompleteTextView(context) {
    ...
    init {
        ...
        for (type in types!!.keys) {
            completeTypes.add(
                    if (Locale.getDefault().language != "en") "$type - " + context.getString(types[type]!!) else type // Translation into all languages other than English
            )
        }
        val listAdapter = ListAdapter(context, android.R.layout.simple_spinner_dropdown_item, completeTypes)
    ...
    internal inner class ListAdapter(context: Context?, piece: Int, strings: List<String?>?) :
            ArrayAdapter<String?>(context!!, piece, strings!!) {
