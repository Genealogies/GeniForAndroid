...
public class EditoreData extends LinearLayout {
		...
		if( Global.settings.expert ) {
			...
			elencoTipi.setOnClickListener( vista -> {
				...
				popup.setOnMenuItemClickListener(item -> {
					datatore.kind = Kind.values()[item.getItemId()];
					...
					if( datatore.kind == Kind.BETWEEN_AND || datatore.kind == Kind.FROM_TO ) {
			...
			findViewById(R.id.editadata_circa).setOnClickListener(vista -> {
				...
				datatore.kind = ((CompoundButton)vista).isChecked() ? Kind.APPROXIMATE : Kind.EXACT;
				veroImputTesto = false;
				genera();
			});
	...
	void impostaTutto() {
		...
		((CheckBox)findViewById( R.id.editadata_circa )).setChecked( datatore.kind == Kind.APPROXIMATE );
		((TextView)findViewById( R.id.editadata_tipi )).setText( dateKinds[datatore.kind.ordinal()] );
